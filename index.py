"""Скрипт для индексации текстовых документов в векторную базу данных Qdrant.

Этот скрипт выполняет следующие шаги:
1. Загружает предобученную модель для генерации эмбеддингов (SentenceTransformer)
2. Подключается к локальному экземпляру Qdrant
3. Создаёт новую коллекцию для векторов
4. Сканирует папку 'documents' и читает все .txt файлы
5. Разбивает каждый документ на чанки по точкам
6. Генерирует векторные представления для каждого чанка
7. Сохраняет чанки с векторами в Qdrant для последующего поиска
"""
