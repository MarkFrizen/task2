...
# Нормализация векторов для косинусного сходства (скалярное произведение)
embeddings_norm = embeddings / np.linalg.norm(embeddings, axis=1, keepdims=True)

# Создание индекса FAISS (Inner Product)
dim = embeddings.shape[1]
index = faiss.IndexFlatIP(dim)
index.add(embeddings_norm)

# Модель для кодирования запроса
model = SentenceTransformer('all-MiniLM-L6-v2')

def search_faiss(query: str, top_k: int = 5):
    """Поиск через FAISS, возвращает список результатов с метаданными."""
